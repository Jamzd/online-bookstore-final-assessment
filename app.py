...
# Global books list to avoid duplication
BOOKS = [
...
def get_book_by_title(title):
    """Return a Book object from BOOKS by title"""
...
def login_required(f):
    """Decorator to enforce login for routes"""
    @wraps(f)
...
def process_checkout():
    """Process checkout with shipping info, payment, and discounts"""
    if cart.is_empty():
        flash('Your cart is empty!', 'error')
        return redirect(url_for('index'))
...
def register():
    if request.method == 'POST':
        ...
        if email.lower() in (u.lower() for u in users):
